    ...
    BorderTabSet.prototype.render = function () {
        ...
        if (border.getLocation() !== DockLocation_1.default.LEFT) {
            for (var i = 0; i < border.getChildren().length; i++) {
                ...
                tabs.push(React.createElement(BorderButton_1.BorderButton, { layout: this.props.layout, border: border.getLocation().getName(), node: child, key: child.getId(), selected: isSelected, iconFactory: this.props.iconFactory, titleFactory: this.props.titleFactory, closeIcon: this.props.closeIcon }));
            }
        }
        else {
